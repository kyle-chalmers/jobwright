...
4. **Deploy.** Run the platform's deploy/update command. The deploy-safety guard prompts before known destructive commands (reset/update/delete/drop/replace and destructive SQL) — but treat steps 1–3 as the real gate, not the prompt, since the guard covers known patterns rather than every possible command.
