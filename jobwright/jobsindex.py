...
# --------------------------------------------------------------------------- #
# Obsidian graph layer — a node per job + a node per data object
# --------------------------------------------------------------------------- #
...
def object_layer(obj: str) -> str:
    ...
    ``ANALYTICS.VW_CUSTOMER`` -> ANALYTICS; ``DB.LEGACY_STORE.MVW`` -> LEGACY_STORE; bare name -> 'object'."""
    parts = obj.split(".")
    return parts[-2] if len(parts) >= 2 else "object"


def graph_stub(tid: str, rows: list[dict], canon: dict) -> str:
    """One id-labeled Obsidian graph node per job ticket. rows = all folders sharing this ticket."""
    r0 = rows[0]
    owners = ", ".join(sorted({r["owner"] for r in rows if r["owner"] and r["owner"] != "—"})) or "—"
    objects = sorted({canon.get(o.lower(), o) for r in rows for o in r["objects"]}, key=str.lower)
    flags = sorted({f for r in rows for f in r["flags"]}, key=str.lower)
    obj_cells = ", ".join(f"[`{o}`](../objects/{object_filename(o)})" for o in objects) or "(none)"
    out = [f"# {tid}: {md_escape(r0['name'])}", "",
           f"`{owners}` · {md_escape(r0['status'])} · {md_escape(r0['last_updated'])}", "",
           f"- **Objects:** {obj_cells}"]
    if flags:  # jobwright-specific: the migration debt shows right on the node
        out.append("- **Deprecated schemas:** " + ", ".join(f"`{md_escape(f)}`" for f in flags))
    for r in rows:
        target = f"../{quote(r['dir'])}/claude.md" if r["has_claude_md"] else f"../{quote(r['dir'])}/"
        label = "docs" if len(rows) == 1 else f"docs ({md_escape(r['dir'])})"
        out.append(f"- **Full job →** [{label}]({target})")
    if r0["purpose"] and r0["purpose"] != "—":
        out += ["", md_escape(r0["purpose"])]
    out += ["", "<!-- generated graph node — regenerated by `jobwright jobs-index`; do not edit -->", ""]
    return "\n".join(out)


def graph_object_note(obj: str, jobs: list) -> str:
    """One node per data object; links the job stubs (../graph/<id>.md). jobs = (tid, name, date)."""
    out = [f"# {obj}", "", f"> `{object_layer(obj)}` layer · touched by **{len(jobs)}** job(s).", ""]
    for tid, name, date in sorted(jobs, key=lambda t: ((t[2] or "0000-00-00"), ref_key(t[0]))):
        suffix = f" — {md_escape(name)}" if name and name != tid else ""
        out.append(f"- [{tid}](../graph/{tid}.md){suffix} ({md_escape(date) or '—'})")
    out += ["", "<!-- generated graph node — regenerated by `jobwright jobs-index`; do not edit -->", ""]
    return "\n".join(out)


def render_graph_layer(rows: list[dict], jobs_root: Path) -> dict[Path, str]:
    """{Path: content} for ``<jobs_dir>/graph/*.md`` + ``<jobs_dir>/objects/*.md``. Deterministic.

    A node per job (its objects + deprecated-schema flags, linking to the folder's docs) and a node
    per data object (the jobs that touch it). Plain relative links — works in stock Obsidian's Graph
    view (no plugins, no wikilinks) and renders on GitHub too. Objects become hubs and jobs cluster
    around the schemas they share, so a deprecated schema shows every job still on it — a live
    migration map."""
    gdir, odir = jobs_root / "graph", jobs_root / "objects"
    by_tid: dict[str, list] = {}
    for r in rows:
        by_tid.setdefault(r["ticket"], []).append(r)
    # Case-fold objects globally (like render_objects) so a mixed-case ref makes ONE note and the
    # stub<->object filenames agree on a case-insensitive filesystem (macOS).
    objmap: dict[str, dict] = {}
    for r in rows:
        for o in r["objects"]:
            slot = objmap.setdefault(o.lower(), {"label": o, "jobs": {}})
            slot["jobs"].setdefault(r["ticket"], (r["ticket"], r["name"], r["last_updated"]))
    canon = {k: v["label"] for k, v in objmap.items()}  # lower(obj) -> canonical display label
    fresh: dict[Path, str] = {}
    for tid, rs in by_tid.items():
        fresh[gdir / f"{tid}.md"] = graph_stub(tid, rs, canon)
    for slot in objmap.values():
        fresh[odir / object_filename(slot["label"])] = graph_object_note(slot["label"], list(slot["jobs"].values()))
    return fresh


def graph_dirs(root: Path, settings: dict) -> list[Path]:
    """The generated graph directories (always cleaned, so turning graph_notes off removes the layer)."""
    jobs_root = root / settings.get("jobs_dir", "jobs")
    return [jobs_root / "graph", jobs_root / "objects"]
...
def render_all(root: Path, settings: dict) -> dict[Path, str]:
    ...
    jobs_root = root / settings.get("jobs_dir", "jobs")
    out: dict[Path, str] = {
        jobs_root / "JOBS.md": render_index(rows),
        jobs_root / "OBJECTS.md": render_objects(rows),
    }
    if settings.get("graph_notes", True):
        out.update(render_graph_layer(rows, jobs_root))
    return out


# --------------------------------------------------------------------------- #
# write / check — shared by the CLI and the PostToolUse hook so both agree
# --------------------------------------------------------------------------- #
def _clean_orphans(fresh: dict[Path, str], dirs: list[Path]) -> None:
    """Remove generated ``*.md`` in the graph dirs no longer in the fresh set; tidy empty dirs."""
    keep = {str(p).lower() for p in fresh}  # case-insensitive for macOS
    for gd in dirs:
        if not gd.is_dir():
            continue
        for existing in sorted(gd.glob("*.md")):
            if str(existing).lower() not in keep:
                existing.unlink()
        try:
            if not any(gd.iterdir()):  # tidy the empty dir (e.g. after disabling graph_notes)
                gd.rmdir()
        except OSError:
            pass


def write_index(root: Path, settings: dict) -> dict[Path, str]:
    """Render + write the catalog and graph layer, removing orphaned graph nodes. Returns the fresh set.
    Writes only changed files (stable mtimes) and is safe to call repeatedly."""
    fresh = render_all(root, settings)
    for p, txt in fresh.items():
        data = txt.encode("utf-8")  # write_bytes => stable \n line endings everywhere
        if not p.is_file() or p.read_bytes() != data:
            p.parent.mkdir(parents=True, exist_ok=True)
            p.write_bytes(data)
    _clean_orphans(fresh, graph_dirs(root, settings))
    return fresh


def stale_index_paths(root: Path, settings: dict) -> list[str]:
    """For ``--check``: repo-relative names of stale catalog files + orphaned graph nodes.
    Empty list == everything up to date."""
    fresh = render_all(root, settings)
    stale = [p.name for p, txt in fresh.items() if (p.read_text() if p.is_file() else None) != txt]
    keep = {str(p).lower() for p in fresh}
    for gd in graph_dirs(root, settings):
        if gd.is_dir():
            stale += [
                p.relative_to(root).as_posix()
                for p in sorted(gd.glob("*.md"))
                if str(p).lower() not in keep
            ]
    return stale
