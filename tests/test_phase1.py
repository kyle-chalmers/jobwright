...
def test_policy_flags_deprecated_schema():
    ...
    pol = ArchitecturePolicy(deprecated_deny=["LEGACY_STORE", "LEGACY_JOBS"], replace_hints={"LEGACY_STORE": "ANALYTICS.VW_CUSTOMER"})
    findings = pol.scan_text("SELECT * FROM LEGACY_STORE.VW_CUSTOMER\n", "x.sql")
    assert len(findings) == 1
    assert findings[0].kind == "deprecated"
    assert findings[0].schema == "LEGACY_STORE"
    assert "ANALYTICS.VW_CUSTOMER" in findings[0].message  # replacement hint surfaced
...
def test_policy_flags_layer_violation_only_when_layer_declared():
    ...
    text = "# LAYER: ANALYTICS\nSELECT * FROM RAW_STORE.EVENTS\n"
    findings = pol.scan_text(text, "x.py")
    assert any(f.kind == "layer-violation" and f.schema == "RAW_STORE" for f in findings)
    # no layer declared -> no layer-violation false positive
    assert not [f for f in pol.scan_text("SELECT * FROM RAW_STORE.EVENTS\n", "y.py") if f.kind == "layer-violation"]
