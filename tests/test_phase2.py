...
def test_scaffolded_job_passes_validation(tmp_path):
    ...
    cfg = _cfg(tmp_path)
    res = new_job(cfg, tmp_path, "JOB-1234", "Demo Outbound List", today="2026-01-01")
    ...
    assert (tmp_path / "databricks/job_definitions/dev/JOB-1234_Demo_Outbound_List.json").is_file()
...
def test_regenerate_index_hook_writes_catalog(tmp_path):
    ...
    (job / "claude.md").write_text("# Job: JOB-7 Thing\n**Purpose**: x\n")
    ...
    assert "JOB-7" in (tmp_path / "jobs" / "JOBS.md").read_text()
