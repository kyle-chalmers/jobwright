...
def test_sql_ddl_scaffold_and_validate(tmp_path):
    ...
    res = new_job(cfg, tmp_path, "JOB-5", "Nightly Rollup", today="2026-01-01")
    # sql-ddl scaffolds a .sql DDL stub, NOT a Databricks JSON
    assert (tmp_path / "defs" / "JOB-5_Nightly_Rollup.sql").is_file()
    assert not (tmp_path / "defs" / "JOB-5_Nightly_Rollup.json").exists()
...
def test_sql_ddl_missing_definition_fails(tmp_path):
    ...
    job = tmp_path / "jobs" / "JOB-6_Thing"
    job.mkdir(parents=True)
    (job / "claude.md").write_text("# Job: JOB-6 Thing\n**Purpose**: x\n**Schedule**: d\n**Business Owner**: t\n")
